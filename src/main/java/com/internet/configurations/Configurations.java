...
public class Configurations {
    ...
    /**
     * Initializes configuration values from the default JSON file and runtime
     * overrides.
     *
     * <p>
     * System properties take precedence over environment variables, which take
     * precedence
     * over values loaded from the configuration file. Defaults are used when no
     * value exists.
     * </p>
     *
    ...
    public static Configurations init() {
        ...
        if (configurations == null) {
            ...
            configurations.setEnv(
                    System.getProperty(
                            "env",
                            Utilities.getEnv("ENV", configurations.getEnv())));
            configurations.setTest(
                    System.getProperty(
                            "test",
                            Utilities.getEnv("TEST", configurations.getTest())));
            configurations.setSuite(
                    System.getProperty(
                            "suite",
                            Utilities.getEnv("SUITE", configurations.getSuite())));
            configurations.setReports(
                    System.getProperty(
                            "reports",
                            Utilities.getEnv("REPORTS", configurations.getReports())));
            configurations.setRetry(
                    Integer.parseInt(
                            System.getProperty(
                                    "retry",
                                    Utilities.getEnv("RETRY", String.valueOf(configurations.getRetry())))));
            configurations.setNavigationTimeout(
                    Duration.ofMillis(Long.parseLong(
                            System.getProperty(
                                    ...
                                    Utilities.getEnv(
                                            "NAVIGATION_TIMEOUT",
                                            String.valueOf(configurations.getNavigationTimeout().toMillis()))))));
            configurations.setExpectedTimeout(
                    Duration.ofMillis(Long.parseLong(
                            System.getProperty(
                                    ...
                                    Utilities.getEnv(
                                            "EXPECTED_TIMEOUT",
                                            String.valueOf(configurations.getExpectedTimeout().toMillis()))))));
            configurations.setActionTimeout(
                    Duration.ofMillis(Long.parseLong(
                            System.getProperty(
                                    ...
                                    Utilities.getEnv(
                                            "ACTION_TIMEOUT",
                                            String.valueOf(configurations.getActionTimeout().toMillis()))))));
            configurations.setPayloadTimeout(
                    Duration.ofMillis(Long.parseLong(
                            System.getProperty(
                                    "page.load.timeout",
                                    Utilities.getEnv(
                                            "PAGE_LOAD_TIMEOUT",
                                            String.valueOf(configurations.getPayloadTimeout().toMillis()))))));
            configurations.setTimeout(
                    Duration.ofMillis(Long.parseLong(
                            System.getProperty(
                                    ...
                                    Utilities.getEnv(
                                            "TIMEOUT",
                                            String.valueOf(configurations.getTimeout().toMillis()))))));
            configurations.setPollingInterval(
                    Duration.ofMillis(Long.parseLong(
                            System.getProperty(
                                    ...
                                    Utilities.getEnv(
                                            "POLLING_INTERVAL",
                                            String.valueOf(configurations.getPollingInterval().toMillis()))))));
        }
